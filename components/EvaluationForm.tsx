  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          <span className={
            hovered ? star <= hovered ? "text-amber-400" : "text-slate-200"
...
export function EvaluationForm({ reservationId, initialRating, initialComment }: Props) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {open && (
        <>
          {/* Backdrop — cierra el popover al hacer clic fuera */}
          <div
            className="fixed inset-0 z-40"
            onClick={() => setOpen(false)}
          />

          {/* Popover flotante — no afecta el layout de la tabla */}
          <div className="absolute right-0 top-full mt-1 z-50 w-60 bg-white border rounded-xl shadow-xl p-3 space-y-2">
